<x-sidebar>
    @section('title', __('Show Blocked Day'))
    <div class="m-4">
        <div class="mb-6">
            <a href="{{ url(route('blocked-days.index')) }}">
                <button
        ...
        <div class="relative z-0 mb-6 w-full group">
            <input type="text" name="name"
                ...
                disabled value="{{ $blocked_day->name }}" />
            <label for="name"
        ...
        <div class="grid md:grid-cols-2 md:gap-6">
            <div class="relative z-0 mb-6 w-full group">
                <input type="text" name="from"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    disabled value="{{ $blocked_day->from }}" />
                <label for="from"
                    class="peer-focus:font-medium absolute text-sm duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 blue-color">
                    {{ __('From') }}
                </label>
    ...
    <div id="editModal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <!-- Modal body -->
                <div class="p-6">
                    <form method="POST" action="{{ route('blocked_days.update', ['blocked_day' => $blocked_day->id]) }}">
                        @csrf
                        @method('PUT')
                        <div class="relative z-0 w-full group flex flex-col">
                            <label for="from"
                                class="mb-2 text-sm font-medium blue-color">{{ __('Name') }}</label>
                            <input type="text" name="name" placeholder="Please enter blocked_day's name"
        ...
        <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow ">
                ...
                <div class="flex justify-end items-center p-6 space-x-2 rounded-b border-t border-gray-200">
                    ...
                    <div>
                        <form method="POST" action="{{ route('blocked_days.destroy', ['blocked_day' => $blocked_day->id]) }}">
                            @csrf
                            @method('DELETE')
